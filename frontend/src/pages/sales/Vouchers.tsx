...
function getTourName(id: string) {
  return mockTourPrograms.find(t => t.id === id)?.name ?? id;
}
...
interface VoucherFormDrawerProps {
  ...
  isEditing?: boolean; // editing existing (vs creating new)
  onClose: () => void;
...
function VoucherFormDrawer({ voucher, isEditing = false, onClose, onSave, onSendApproval, initialStatus }: VoucherFormDrawerProps) {
  const [form, setForm] = useState<Partial<Voucher>>(
    voucher ?? {
      code: '', type: 'percent', value: '', startDate: '', endDate: '',
      limit: undefined as unknown as number, applicableTours: [], status: initialStatus ?? 'draft', description: '',
    }
  );
  ...
  const canEdit = !voucher || ['draft', 'rejected'].includes(voucher.status ?? initialStatus ?? 'draft');
  ...
  const validate = () => {
    ...
    if (form.limit === undefined || form.limit === null || isNaN(Number(form.limit)) || Number(form.limit) <= 0) {
      e.limit = 'Số lượng phải lớn hơn 0';
    }
    setErrors(e);
  ...
  const handleSave = () => {
    if (!validate()) return;
    const saved = { ...(voucher ?? {}), ...form, applicableTours: selectedTourIds } as Voucher;
    // If saving from rejected → status becomes draft
  ...
  const handleSendApproval = () => {
    if (!validate()) return;
    onSendApproval?.({ ...(voucher ?? {}), ...form, applicableTours: selectedTourIds, status: 'pending_approval' as VoucherStatus } as Voucher);
  };

  const showSendApproval = canEdit && (voucher?.status === 'draft' || voucher?.status === 'rejected');
        ...
        <div className="w-full h-full bg-white shadow-2xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {isEditing && form.status && (
              <div>
                <span className={`inline-block px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded ${STATUS_STYLE[form.status]}`}>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  disabled={!canEdit}
                  className={`w-full border p-3 text-sm outline-none ${errors.startDate ? 'border-red-400' : 'border-[#D0C5AF]/40 focus:border-[#D4AF37]'} ${!canEdit ? 'bg-gray-50 cursor-not-allowed' : ''}`}
                />
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  disabled={!canEdit}
                  className={`w-full border p-3 text-sm outline-none ${errors.endDate ? 'border-red-400' : 'border-[#D0C5AF]/40 focus:border-[#D4AF37]'} ${!canEdit ? 'bg-gray-50 cursor-not-allowed' : ''}`}
                />
            ...
            <div>
              ...
              <div className="relative">
                <button
                  type="button"
                  onClick={() => canEdit && setTourDropdownOpen(o => !o)}
                  disabled={!canEdit}
                ...
                >
                  ...
                  <span className={`material-symbols-outlined text-[16px] transition-transform ${tourDropdownOpen ? 'rotate-180' : ''}`}>
                    expand_more
                  </span>
                </button>
                {tourDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-[#D0C5AF]/40 shadow-lg max-h-48 overflow-y-auto">
                    <button
                      type="button"
                      onClick={() => { setSelectedTourIds([]); setTourDropdownOpen(false); }}
                    ...
                    {mockTourPrograms.map(tp => (
                      <button
                        key={tp.id}
                        type="button"
                        onClick={() => { toggleTour(tp.id); setTourDropdownOpen(false); }}
...
function VoucherDetailDrawer({ voucher, onClose, onApprove, onReject, onEdit, onSendApproval, role }: VoucherDetailDrawerProps) {
        ...
        <div className="w-full h-full bg-white shadow-2xl flex flex-col">
              ...
              {voucher.applicableTours.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {voucher.applicableTours.map(t => (
                    <span key={t} className="px-3 py-1 bg-[#D4AF37]/10 text-[#D4AF37] text-xs font-bold rounded-full border border-[#D4AF37]/20">{getTourName(t)}</span>
                  ))}
                </div>
              ) : (
          ...
          <div className="p-6 border-t border-[#D0C5AF]/30 bg-[#FAFAF5] flex gap-3 shrink-0">
            ...
            {canEdit && voucher.status === 'rejected' && (
              <button
                onClick={() => onSendApproval?.(voucher)}
                className="flex-1 py-3 text-xs font-['Inter'] uppercase tracking-widest font-bold bg-[#2A2421] text-white hover:bg-[#D4AF37] transition-colors"
              >
                Gửi Phê Duyệt
              </button>
...
export default function VoucherManagement() {
  ...
  const ALL_STATUSES: VoucherStatus[] = ['draft', 'pending_approval', 'rejected', 'active', 'inactive'];
                    ...
                    {v.applicableTours.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {v.applicableTours.slice(0, 2).map(t => (
                          <span key={t} className="px-1.5 py-0.5 bg-[#D4AF37]/10 text-[#D4AF37] text-[10px] font-bold rounded max-w-[100px] truncate" title={getTourName(t)}>
                            {getTourName(t)}
                          </span>
                        ))}
