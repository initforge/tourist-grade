...
function ReasonPopup({ title, onConfirm, onCancel }: { title: string; onConfirm: (reason: string) => void; onCancel: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="estimate-reason-title" className="relative bg-white w-full max-w-sm mx-4 shadow-2xl border border-[#D0C5AF]/30 p-8">
        <h3 id="estimate-reason-title" className="font-['Noto_Serif'] text-xl text-[#2A2421] mb-2">{title}</h3>
...
function ApproveConfirmPopup({ instanceName, onConfirm, onCancel }: { instanceName: string; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-[#2A2421]/50 backdrop-blur-sm" onClick={onCancel}></div>
      <div role="dialog" aria-modal="true" aria-labelledby="estimate-approve-title" className="relative bg-white w-full max-w-sm mx-4 shadow-2xl border border-[#D0C5AF]/30 p-8">
        <div className="w-14 h-14 rounded-full bg-emerald-50 flex items-center justify-center mx-auto mb-4">
          <span className="material-symbols-outlined text-3xl text-emerald-500">check_circle</span>
        </div>
        <h3 id="estimate-approve-title" className="font-['Noto_Serif'] text-xl text-[#2A2421] text-center mb-2">Duyệt dự toán</h3>
