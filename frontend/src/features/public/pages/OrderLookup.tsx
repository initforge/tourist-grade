...
import { mockBookings, type Booking } from '@entities/booking/data/bookings';
...
export default function OrderLookup() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const normalizedCode = bookingCode?.trim()?.toLowerCase();
    const normalizedContact = normalizeLookupValue(phone);

    const found = mockBookings?.find(b =>
      b?.bookingCode?.toLowerCase() === normalizedCode &&
      (
        normalizeLookupValue(b?.contactInfo?.phone) === normalizedContact ||
        b?.contactInfo?.email?.trim()?.toLowerCase() === phone?.trim()?.toLowerCase()
      )
    );
    if (found) {
  ...
  return (
    <div className="w-full bg-[var(--color-background)] min-h-[80vh] pt-16 md:pt-20 pb-20 md:pb-24 px-4 sm:px-6">
      ...
      >
        <section aria-label="Form tra cứu đơn đặt" className="w-full bg-white border border-[#D0C5AF]/40 shadow-sm p-10 animate-in fade-in duration-500 delay-150">
          <form onSubmit={handleSearch} className="space-y-8">
            ...
            {notFound && (
              <div className="bg-red-50 border border-red-200 text-red-700 text-sm p-4 text-center rounded-sm">
                Không tìm thấy đơn đặt chỗ?. Vui lòng kiểm tra lại mã đơn và thông tin liên hệ?.
              </div>
            )}
        ...
        {foundBooking && (
          <section aria-label="Kết quả tra cứu đơn đặt" className="w-full animate-in slide-in-from-bottom-5 duration-500">
      ...
      {showCancelModal && foundBooking && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div
            ...
            className="relative w-full max-w-lg max-h-[90vh] bg-white shadow-2xl flex flex-col animate-in zoom-in-95 duration-300"
          >
            {/* Header */}
            <div className="p-6 pb-5 border-b border-outline-variant/20 shrink-0">
              <div className="flex items-center justify-between">
                <h3 id="order-cancel-title" className="font-serif text-xl text-primary">Gửi yêu cầu hủy</h3>
            ...
            <div className="p-6 space-y-5 overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input className="border border-outline-variant/50 px-3 py-2.5 text-sm focus:border-[var(--color-secondary)] outline-none" placeholder="Ngàn hàng" />
              ...
              <label className="flex items-start gap-3 cursor-pointer text-sm text-primary/70">
                ...
                <span>Tôi đồng ? với chành sách hoàn hủy</span>
              </label>
            </div>
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 id="order-payment-title" className="font-serif text-xl text-primary">Thanh toán</h3>
