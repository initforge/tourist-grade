...
const TABS: { key: TabKey; label: string; desc: string }[] = [
  ...
  { key: 'phan_cong_hdv', label: 'Phân công HDV', desc: 'Tour sẵn sáng phân công hướng dẫn viên' },
...
// Các tab đã qua bước phân công thẻ hiển thị HDV thay cho người tạo?.
...
const COMMON_COLS_HDV = ['Mã tour', 'T?n chương trình', 'Ngày KH', 'Điểm KH', 'Điểm TQ', 'Thời lượng', 'Số KH', 'Hướng dẫn viên', 'Trạng thái', ''];

const COL_COMPLETED = ['Mã tour', 'T?n chương trình', 'Ngày KH', 'Số KH', 'Doanh thu thực tế', 'Chi phí thực tế', 'Lợi nhuận', ''];
const COL_CANCELLED = ['Mã tour', 'Tên chương trình', 'Ngày KH', 'Số KH đăng k?', 'Thời điểm hủy', 'Tổng tiền hoàn', 'Lý do'];
...
export default function AdminTourPrograms() {
  ...
  const getStatusLabel = (inst: TourInstance, tab: TabKey) => {
    ...
    return TOUR_INSTANCE_STATUS_LABEL[inst?.status];
  };
  ...
  const renderCell = (inst: TourInstance, col: string, tab: TabKey) => {
    ...
    if (tab === 'da_huy') {
      ...
      if (col === 'Tên chương trình') return inst?.programName;
      if (col === 'Ngày KH') return new Date(inst?.departureDate)?.toLocaleDateString('vi-VN');
      if (col === 'Số KH đăng k?') return inst?.expectedGuests;
    ...
    if (col === 'Thời lượng') {
      const program = getProgram(inst?.programId);
    ...
    if (col === 'Người tạo') return (
      <span className="text-xs text-primary/50">{inst?.createdBy}</span>
    );
        ...
        <div className="bg-white border-x border-b border-outline-variant/30 overflow-hidden">
          {filtered.length === 0 ? (
            <div className="py-20 text-center">
              ...
              <p className="text-sm text-primary/40">Khàng có tour nào trong mục này</p>
            </div>
          ) : (
