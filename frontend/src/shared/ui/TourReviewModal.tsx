...
export function TourReviewModal({ bookingId, tourId, onClose }: TourReviewModalProps) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (comment.trim().length < 10 || submitting) {
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <form onSubmit={submit} className="relative z-10 w-full max-w-lg bg-white shadow-2xl p-8 space-y-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="font-serif text-2xl text-primary">Đánh giá tour</h2>
            <p className="text-sm text-primary/55 mt-1">Chia sẻ trải nghiệm của bạn sau chuyến đi.</p>
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            {Array.from({ length: 5 }).map((_, index) => {
              const star = index + 1;
              return (
                <button key={star} type="button" onClick={() => setRating(star)} className="text-secondary">
        ...
        <label className="block space-y-2">
          ...
          <input value={title} onChange={(event) => setTitle(event.target.value)} className="w-full border border-outline-variant/50 px-4 py-3 text-sm" placeholder="Điều bạn ấn tượng nhất" />
        </label>

        <label className="block space-y-2">
          <span className="text-xs font-medium text-primary/70">Nội dung đánh giá</span>
          <textarea value={comment} onChange={(event) => setComment(event.target.value)} className="w-full border border-outline-variant/50 px-4 py-3 text-sm min-h-[140px] resize-none" placeholder="Chia sẻ chi tiết trải nghiệm của bạn" />
        </label>
